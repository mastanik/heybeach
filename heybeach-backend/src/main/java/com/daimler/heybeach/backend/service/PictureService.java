...
public class PictureService {
    ...
    public void like(Long id) throws PictureException, NotFoundException, ValidationException {
        try {
            ...
            if (likeCount >= marketplaceEntranceLikeAmount) {
                ...
                if (PictureStatus.getById(picture.getPictureStatus()) == PictureStatus.APPROVED) {
                    ...
                    pictureStatusLogDao.save(pictureStatusLog);
                }
            }
    ...
    public void disapprove(Long id, ApproveDto dto) throws PictureException, NotFoundException, ValidationException {
        try {
            LoggedInUser user = (LoggedInUser) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
            Picture picture = pictureDao.findById(id);
            ...
            pictureDao.save(picture);
            createPictureStatusLog(id, user.getUserId(), dto.getComment(), pictureStatus);
        } catch (DaoException e) {
    ...
    public void approve(Long id, ApproveDto dto) throws PictureException, NotFoundException, ValidationException {
        try {
            LoggedInUser user = (LoggedInUser) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
            Picture picture = pictureDao.findById(id);
            PictureStatus pictureStatus = PictureStatus.getById(picture.getPictureStatus());
            ...
            pictureDao.save(picture);
            createPictureStatusLog(id, user.getUserId(), dto.getComment(), pictureNextStatus);
    ...
    private void createPictureStatusLog(Long pictureId, Long userId, String comment, PictureStatus pictureStatus) throws DaoException, ValidationException {
        PictureStatusLog pictureStatusLog = new PictureStatusLog();
    ...
    public void remove(Long id) throws PictureException, NotFoundException, ValidationException {
        try {
            LoggedInUser user = (LoggedInUser) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
